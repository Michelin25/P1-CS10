...
public class DotTreeGUI extends InteractiveGUI {
	...
	public void drawTree(Graphics g, PointQuadtree<Point> tree, int level) {
		...
		if (tree.hasChild(4)) drawTree(g,tree.getChild(1),level);
		if (tree.hasChild(3)) drawTree(g,tree.getChild(1), level);
		if (tree.hasChild(2)) drawTree(g,tree.getChild(1), level);
